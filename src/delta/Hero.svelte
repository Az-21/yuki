<script lang="ts">
  ...
  let hiddenIndicator: string = '[ - ]';

  function hideSection(): void {
    hidden = !hidden;
    if (hidden) hiddenIndicator = '[ + ]';
    else hiddenIndicator = '[ - ]';
  }
</script>

<div
  class="{spacing} prose prose-invert w-full max-w-none rounded-lg bg-neutral-900 px-8 py-4 shadow-md">
    ...
    <div class="prose prose-invert w-full max-w-none prose-a:text-blue-400">
      <p>
        YUKI provides a collection of essential software for your PC and Chromium based browsers. <code
          >winget</code> package manager is highly recommended to install and update apps. Website links
        are also available if you prefer the old school way.
      </p>
